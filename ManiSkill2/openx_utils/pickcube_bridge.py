...
natural_instruction = "pick up the red cube and move it to the green point"
...
class PickCubeMoreCameraEnv(PickCubeEnv):
    ...
    def check_frame_goal_visibility(self, obs, min_num_pixel=10):
        names_of_actors_to_be_visible = ["goal_site", "cube"]
...
def process(params):
    ...
    if SAVE_TO_CEPH:
        ...
        for camera_save_idx in range(NUM_CAMERAS):
            file_name = osp.join(CEPH_SAVE_BUCKET, env_id, file_name_base + str(camera_save_idx) + ".pkl")
