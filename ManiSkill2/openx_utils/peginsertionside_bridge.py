...
from mani_skill2.utils.sapien_utils import look_at, vectorize_pose, hex2rgba
...
natural_instruction = "insert the peg into the horizontal hole in the box"
...
@register_env("PegInsertionSide-v0-MoreCamera", max_episode_steps=200, override=True)
class PegInsertionSideMoreCameraEnv(PegInsertionSideEnv):
    def __init__(self, *args, camera_params=None, with_goal_site=False, **kwargs):
        self.camera_params = camera_params
        self.with_goal_site = with_goal_site
    ...
    def _load_actors(self):
        ...
        self.box_hole_bound_offset_1 = Pose(np.hstack([-length+radius, center]))
        self.box_hole_bound_offset_2 = Pose(np.hstack([length-radius, center]))
        
        if self.with_goal_site:
            self.goal_site = self._build_sphere_site(radius=radius)
    
    def _initialize_task(self, *args, **kwargs):
        super()._initialize_task(*args, **kwargs)
        if self.with_goal_site:
            goal_pos_1 = self.box.pose.transform(self.box_hole_bound_offset_1).p
            goal_pos_2 = self.box.pose.transform(self.box_hole_bound_offset_2).p
            peg_head_pose = self.peg_head_pos
            diff_1 = np.linalg.norm(goal_pos_1 - peg_head_pose)
            diff_2 = np.linalg.norm(goal_pos_2 - peg_head_pose)
            if diff_1 < diff_2:
                goal_pos = goal_pos_1
            else:
                goal_pos = goal_pos_2
    ...
    def check_frame_goal_visibility(self, obs, min_num_pixel=10):
        assert self.with_goal_site, "Goal site is not available"
        
        names_of_actors_to_be_visible = ["peg", "goal_site"]
    ...
    while try_count < MAX_NUM_TRY:
        try:
            ...
            while len(valid_camera_params) < NUM_CAMERAS:
                ...
                env_kwargs["obs_mode"] = "rgbd"
                env_kwargs["control_mode"] = TARGET_CONTROL_MODE
                env = gym.make(multi_camear_env_id, camera_params=camera_poses, with_goal_site=True, render_mode="cameras", **env_kwargs)
            ...
            seed = reset_kwargs.pop("seed")
            reward = 0.0
            obs_with_goal, _ = env.reset(seed=seed, options=reset_kwargs)
            env.goal_site.hide_visual()
            ...
            for i in tqdm(range(len(target_ee_poses))):
                ...
                goal_visible_camera_names = env.check_frame_goal_visibility(obs_with_goal, min_num_pixel=MIN_NUM_PIXELS_TO_BE_VISIBLE*(CAMERA_RESOLUTION_SCALE**2))
                ...
                obs_with_goal, reward, terminated, _, info = env.step(action)
                env.goal_site.hide_visual()
                obs = env.observation(env.get_obs())
