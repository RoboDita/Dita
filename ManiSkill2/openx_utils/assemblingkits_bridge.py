...
natural_instruction = "insert the object into the corresponding slot on the plate"
...
@register_env("AssemblingKits-v0-MoreCamera", max_episode_steps=200, override=True)
class AssemblingKitsMoreCameraEnv(AssemblingKitsEnv):
    ...
    def _load_actors(self, *args, **kwargs):
        super()._load_actors(*args, **kwargs)
        if self.with_goal_site:
            self.goal_site = self._build_sphere_site(radius=0.025)
    
    def _initialize_task(self, *args, **kwargs):
        super()._initialize_task(*args, **kwargs)
        if self.with_goal_site:
            goal_pos = self.objects_pos[self.object_id].copy()
            goal_pos[2] = 0.01
    ...
    def check_frame_goal_visibility(self, obs, min_num_pixel=10):
        ...
        names_of_actors_to_be_visible = ["kit", f"obj_{self.object_id:02d}", "goal_site"]
