...
@register_env("PickSingleYCB-v0-MoreCamera", max_episode_steps=200, override=True)
class PickSingleYCBMoreCameraEnv(PickSingleYCBEnv):
    ...
    def check_frame_goal_visibility(self, obs, min_num_pixel=10):
        names_of_actors_to_be_visible = ["goal_site", self.obj.name]
...
def process(params):
    episode_idx, random_seed, camera_pool_range, traj_model_name, natural_instruction, h5_file, json_data, client = params
    random.seed(random_seed)
    np.random.seed(random_seed)
    
    log_print("[START] processing episode ", episode_idx + 1)
    
    file_name_base = env_id + "_" + traj_model_name + "_traj_" + str(episode_idx) + "_camera_"
    ...
    while try_count < MAX_NUM_TRY:
        try:
            ...
            env = gym.make(env_id, render_mode="cameras", **env_kwargs)
            ...
            while len(valid_camera_params) < NUM_CAMERAS:
                ...
                ori_env_kwargs = json_data["env_info"]["env_kwargs"]
                ori_env = gym.make(multi_camear_env_id, camera_params=camera_poses, **ori_env_kwargs)
                env_kwargs = ori_env_kwargs.copy()
                env_kwargs["obs_mode"] = "rgbd"
                env_kwargs["control_mode"] = TARGET_CONTROL_MODE
                env = gym.make(multi_camear_env_id, camera_params=camera_poses, render_mode="cameras", **env_kwargs)
...
def process_wrapper(params):
    train_traj_ranges_this_proc, random_seeds_this_proc, camera_pool_range, traj_model_name, natural_instruction, traj_path, json_path = params
    ...
    for i in range(num_trajs):
        flag = process((train_traj_ranges_this_proc[i], random_seeds_this_proc[i], camera_pool_range, traj_model_name, natural_instruction, h5_file, json_data, client))
...
def main():
    ...
    parser.add_argument("--start_model_index", type=int, default=0)
    parser.add_argument("--end_model_index", type=int, default=10)
    args = parser.parse_args()
    
    NUM_PROC = args.num_proc
    START_MODEL_INDEX = args.start_model_index
    END_MODEL_INDEX = args.end_model_index
    ...
    traj_fns_to_run = traj_fns[START_MODEL_INDEX:END_MODEL_INDEX]
    log_print("Processing models from index {} to {}".format(START_MODEL_INDEX, END_MODEL_INDEX))
    
    data_split_info_all_object = {}
    
    for fn_idx, fn in enumerate(traj_fns_to_run):
        log_print("Processing file ", fn_idx + 1, " / ", len(traj_fns_to_run), ":", fn)
        
        traj_model_name = fn
        natural_instruction = natural_instruction_template.format(convert_fn_to_model_category(fn))
        ...
        params = (traj_model_name, natural_instruction, traj_path, json_path)
        ...
        if NUM_PROC > 1:
            ...
            for i_proc in range(NUM_PROC):
                result = pool.apply_async(process_wrapper, args=((train_traj_range_parts[i_proc], random_seeds_parts[i_proc], train_camera_pool_range) + params, ))
            ...
            new_traj_ids, exist_traj_ids, partial_traj_ids, failed_traj_ids = process_wrapper((np.arange(train_traj_range[0], train_traj_range[1]), random_seeds[train_traj_range[0]:train_traj_range[1]], train_camera_pool_range) + params)
            new_traj_ids_all.extend(new_traj_ids)
            exist_traj_ids_all.extend(exist_traj_ids)
            partial_traj_ids_all.extend(partial_traj_ids)
            failed_traj_ids_all.extend(failed_traj_ids)
        ...
        if NUM_PROC > 1:
            ...
            results = []
            for i_proc in range(NUM_PROC):
                result = pool.apply_async(process_wrapper, args=((val_traj_range_parts[i_proc], random_seeds_parts[i_proc], val_camera_pool_range) + params, ))
                results.append(result)
            for result in results:
                output = result.get()
                new_traj_ids, exist_traj_ids, partial_traj_ids, failed_traj_ids = output
            ...
            log_print("Processing trajectories sequentially")
            new_traj_ids, exist_traj_ids, partial_traj_ids, failed_traj_ids = process_wrapper((np.arange(val_traj_range[0], val_traj_range[1]), random_seeds[val_traj_range[0]:val_traj_range[1]], val_camera_pool_range) + params)
            new_traj_ids_all.extend(new_traj_ids)
            exist_traj_ids_all.extend(exist_traj_ids)
            partial_traj_ids_all.extend(partial_traj_ids)
            failed_traj_ids_all.extend(failed_traj_ids)
        ...
        data_split_info_all_object[traj_model_name] = data_split_info
    
    with open(f"{env_id}_{START_MODEL_INDEX}_{END_MODEL_INDEX}_gen_data_info.json", 'w') as f:
        json.dump(data_split_info_all_object, f, indent=4)
